...
double xi(double *rho, double *r, int i, double rho_b){
	int j,k;
	// smoothed density approximation (SDA)
	double rho_sj[nstep];
	...
	for (j=0; j<nstep; j++) {
		for (k=0; k<nrmesh; k++) {
			...
			rho_dfex_int_k[k] = rho[j]*dfex_per_drhos(rho_sj[j])*drhos_per_drho_j(ra, rho_sj[j], rho_s1j[j], rho_s2j[j])*(2.0*M_PI*(double(k)*drc));
			rho_phi_int_k[k]  = rho[j]*phi_att(ra)*(2.0*M_PI*(double(k)*drc));
		}
		//ingegral_simpson(double *f, int n, double dx)
		rho_dfex_int_j[j] = ingegral_simpson(rho_dfex_int_k, nrmesh, drc);
