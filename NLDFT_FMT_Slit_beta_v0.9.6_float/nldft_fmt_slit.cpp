...
void read_parameters(void){
	...
	//dd = 2.0*d_hs/float(ndmesh-1); // rho_si(), xi()
	ndmesh = nrmesh;
	...
	for (i=0; i<=(nstep-2)/2; i++){
		for (j=0; j<nstep; j++) {
			...
			for (k=1; k<nrmesh; k++) {
				...
				ra = std::sqrt(ra);
				//std::cout << ra << std::endl;
				//alpha_int_k[k]  = -phi_att(ra)*(2.0*M_PI*(float(k)*drc));
	...
	for (i=0; i<nstep; i++) {
		for (j=0; j<nstep; j++) {
			...
			for (k=1; k<nrmesh; k++) {
				...
				ra = std::sqrt(ra);
				//std::cout << ra << std::endl;
				//rho_phi_int_k[k]  = rho[j]*phi_att(ra)*(2.0*M_PI*(float(k)*drc)); // old ver.1.1.0
...
float xi(float *rho, float *r, int i, float rho_b, float *phi_att_int_ij, float *rho_phi_int){
	int j,k;
	float ra;
	float raj;
	float rak;
	...
	float rho_phi_int_k[nrmesh]; // old ver.1.1.1
	rho_phi_int_k[0] = 0.0;
	for (j=0; j<nstep; j++) {
		//raj = (r[i]-r[j]);
		//for (k=1; k<ndmesh; k++) {
		//	rak = dd*float(k);
			//ra = std::pow((r[i]-r[j]),2.0) + std::pow((float(k)*dd),2.0);
			//ra = (r[i]-r[j])*(r[i]-r[j]) + (float(k)*dd)*(float(k)*dd);
		//	ra = raj*raj + rak*rak;
			//ra = std::pow(ra,0.5);
		//	ra = std::sqrt(ra);
			//std::cout << ra << std::endl;
			...
			//rho_dfex_int_k[k] = rho[j]*dfex_per_drhos(rho_sj[j])*drhos_per_drho_j(ra, rho_sj[j], rho_s1j[j], rho_s2j[j])*(2.0*M_PI*(float(k)*dd)); // old ver.1.1.0
			//rho_dfex_int_k[k] = drhos_per_drho_j(ra, rho_sj[j], rho_s1j[j], rho_s2j[j])*(2.0*M_PI*(float(k)*dd));
			//rho_dfex_int_k[k] = drhos_per_drho_j(ra, rho_sj[j], rho_s1j[j], rho_s2j[j])*(tpidd*float(k));
		//}
		//integral_simpson(float *f, int n, float dx)
		...
		//	//std::cout << ra << std::endl;
		//	//rho_phi_int_k[k]  = rho[j]*phi_att(ra)*(2.0*M_PI*(float(k)*drc)); // old ver.1.1.0
		//	rho_phi_int_k[k]  = phi_att(ra)*(2.0*M_PI*(float(k)*drc));
		//} // old ver.1.1.1
		//integral_simpson(float *f, int n, float dx)
...
int main(){
	...
	//
	for (i=0; i<nstep; i++){
		// r[i] = sigma_ss/2.0 + (H-sigma_ss)/float(nstep)*float(i);
		// 1.72 times is escape nan, etc from positive value of wall potential
		//r[i] = sigma_ss*1.74/2.0 + dr*float(i) + dr/2.0; // dr = (H-sigma_ss*1.74)/float(nstep+1);
