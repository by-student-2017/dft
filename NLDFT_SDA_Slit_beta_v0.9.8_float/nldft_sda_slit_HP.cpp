...
int main(){
	...
	for (i=0; i<nstep; i++){
		...
		if (-phi_ext_i[i] > 0.0 && T > 273.0 && pp0 > 1e7){
			rho[i] = 2.0*rho_b0/dr*-phi_ext_i[i]/2000.0*1e-4;
		}
		rho_new[i] = 0.0;
	}
	//
	...
	//
	//high pressure
	float rho_b_k[89]={4.59813e-07,7.65e-07,1.48e-06,2.78e-06,5.07e-06,8.99e-06,1.55e-05,2.61e-05,4.28e-05,6.87e-05,0.000107744,
						0.000165450,0.000249000,0.000367617,0.000532901,0.000759151,0.001063641,0.001466842,0.001992605,0.002668158,0.003524105,
						0.004594237,0.005915211,0.007526184,0.009468211,0.011783671,0.014515526,0.017706579,0.021398421,0.025631184,0.030442237,
						0.035865395,0.041930789,0.048663553,0.056083947,0.064206711,0.073040395,0.082588289,0.092847105,0.103808026,0.115456447,
						0.127772237,0.140730263,0.154301316,0.168452632,0.183144737,0.198336842,0.213988158,0.230053947,0.246484211,0.263235526,
						0.280259211,0.297506579,0.314930263,0.332486842,0.350127632,0.367810526,0.385494737,0.403138158,0.420705263,0.438159211,
						0.472598684,0.506219737,0.538827631,0.570261842,0.600400000,
						0.629153947,0.656461842,0.682292105,0.706632895,0.729494737,
						0.750900000,0.770886842,0.789501316,0.806798684,0.822838158,
						0.837682895,0.851397368,0.864050000,0.875705263,0.886427632,
						0.896281579,0.928177632,
						0.954965789,
						0.971913158,
						0.982542105,
						0.989169737,
	...
	for (k=0; k<=89; k++){
		...
		for (j=0; j<cycle_max; j++){
			...
			for (i=0; i<=(nstep-2)/2; i++){
				...
				if (-14 < xio && xio < 12){
				    ...
				    // to avoid overflow
					rho_new[i] = (0.001*rho_b0/dr + rho[i])*1.1;
				}
			}
			//
			diff_old1 = diff;
		...
		// The Carnahan-Starling equation of state is not applicable for packing fractions greater than 0.55.
